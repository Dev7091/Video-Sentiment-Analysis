...
UPLOAD_FOLDER = 'D:/College Files/Video-Sentiment-Analysis/static/'
...
def upload_file():
    if request.method == 'POST':
        # Close any opened files explicitly
        for filename in os.listdir('static/'):
        ...
        for filename in os.listdir('static/'):
            if filename.startswith('projectwithaudio'):
                try:
                    os.remove('static/' + filename)
                except PermissionError:
                    pass
...
def multi_modal():
    pathvideo = None  # Initialize pathvideo
    pathtext = None  # Initialize pathtext
    ...
    for file in request.files.getlist('mymmfile'):
        if file.filename.endswith(".mp4"):
            ...
            if file.filename.endswith((".vtt", ".srt")):  # Check for both '.vtt' and '.srt' extensions
                pathtext = os.path.join(app.config['UPLOAD_FOLDER'], "subtitles.vtt")
                file.save(pathtext)

    if pathtext is not None:  # Check if pathtext is not None
        for filename in os.listdir('static/'):
        ...
        # Render the template without timestamps when pathtext is None
        return render_template('multimodal.html', error_message="Subtitle file is missing or invalid")
